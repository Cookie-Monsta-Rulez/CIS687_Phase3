...
std::unique_ptr<std::ofstream> FileManagement::openTemp(const std::string& fileName) const
{
    fs::path path = tempDir / fileName;
    auto out = std::make_unique<std::ofstream>(path, std::ios::out | std::ios::binary | std::ios::app);
    if (!out->is_open())
